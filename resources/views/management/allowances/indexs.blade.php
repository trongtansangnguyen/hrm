...
<div class="bg-white rounded-lg shadow-sm p-6">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('management.allowances.creates') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-sm">
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody>
            ...
            <tr class="border-b hover:bg-gray-50">
                <td class="p-3 text-sm text-gray-700">{{ $type->id }}</td>
                <td class="p-3 text-sm text-gray-700 font-medium">{{ $type->name }}</td>
                <td class="p-3 text-sm text-gray-700">{{ number_format($type->amount) }} VNĐ</td>
